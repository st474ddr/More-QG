...
file = io.open ("english_result.txt" ,"w")
io.output(file)
  ...
  while true do
    ...
    if srcTokens == nil or #srcBatch == opt.batch_size then
      ...
      for b = 1, #predBatch do
        ...
        --io.write("test")
        --io.close(file)
        if info[b].score > -10  then
          io.write('\nSENT ' .. sentId .. ': ' .. srcSent)
          io.write('\nPRED ' .. sentId .. ': ' .. predSent)
          io.write(string.format("\nPRED SCORE: %.4f     ", info[b].score))
       end
        ...
        if opt.n_best > 1 then
          ...
          for n = 1, #info[b].nBest do
            ...
            io.write(string.format("[%.4f] %s", info[b].nBest[n].score, nBest))
          end
        end
        io.write('')
        sentId = sentId + 1
